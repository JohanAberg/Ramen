...
{
	if( render_splash( ramen::app().system().resources_path()))
	{
	...
	{
        boost::filesystem::path img_path( ramen::app().system().resources_path());
        img_path /= "splash.jpg";
	    QPixmap pixmap( img_path.string().c_str());
	    splash_ = new splash_screen_widget_t( pixmap);
	}
}
...
bool splash_screen_t::render_splash( const boost::filesystem::path& resources_path)
{
	boost::filesystem::path splash_sexpr_path = resources_path / "splash.se";
...
bool splash_screen_t::do_render_splash( const boost::filesystem::path& p)
{
	try
	{
		...
		std::ifstream istream( ramen::filesystem::file_string( p).c_str());
		if( !istream)
			return false;
		...
		for( int j = 0; j < image_height(); ++j)
		{
			for( int i = 0; i < image_width(); ++i)
			{
				...
				image_.setPixel( i, j, qRgb( Imath::clamp( result[0], 0.0, 1.0) * 256,
											 Imath::clamp( result[1], 0.0, 1.0) * 256,
											 Imath::clamp( result[2], 0.0, 1.0) * 256));
			}
		}
