...
#ifndef RAMEN_ANIM_DOUBLE_KEY_HPP
#define RAMEN_ANIM_DOUBLE_KEY_HPP
...
class double_key_t : public keyframe_t
{
public:
    ...
    double_key_t();
    double_key_t( time_type time, value_type value);

	void swap( double_key_t& other);
	void swap_value( double_key_t& other);
    ...
    void calc_tangents( const double_key_t *prev, const double_key_t *next);

    void set_v0_tangent( value_type slope);
    void set_v1_tangent( value_type slope);

    // cubic
    void calc_cubic_coefficients( const double_key_t& next);
