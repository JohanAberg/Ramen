...
public:
	...
	{
		keys().erase( adobe::remove_if( keys(), boost::bind( &key_type::selected, _1)), keys().end());
	}
	...
	{
		adobe::for_each( keys(), boost::bind( &key_type::select, _1, b));
	}
	
	bool any_keyframe_selected() const
	{
		return adobe::find_if( keys(), boost::bind( &key_type::selected, _1 )) != keys().end();
	}
