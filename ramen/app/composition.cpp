...
{
	adobe::for_each( nodes(), boost::bind( &node_t::set_notify_dirty, _1, false));
}

void composition_t::begin_interaction()
{
	adobe::for_each( nodes(), boost::bind( &node_t::begin_interaction, _1));
}

void composition_t::end_interaction()
{
	clear_all_notify_dirty_flags();
	adobe::for_each( nodes(), boost::bind( &node_t::end_interaction, _1));
	notify_all_dirty();
}
...
// selections
void composition_t::select_all() { adobe::for_each( nodes(), boost::bind( &node_t::select, _1, true));}

void composition_t::deselect_all() { adobe::for_each( nodes(), boost::bind( &node_t::select, _1, false));}
...
{
	adobe::for_each( nodes(), boost::bind( &node_t::convert_relative_paths, _1, composition_dir_, new_base));
}

void composition_t::make_all_paths_absolute()
{
	adobe::for_each( nodes(), boost::bind( &node_t::make_paths_absolute, _1));
}

void composition_t::make_all_paths_relative()
{
	adobe::for_each( nodes(), boost::bind( &node_t::make_paths_relative, _1));
}
...
void composition_t::write( serialization::yaml_oarchive_t& out) const
{
    ...
    out << YAML::Key << "nodes" << YAML::Value;
		out.begin_seq();
		    adobe::for_each( nodes(), boost::bind( &node_t::write, _1, boost::ref( out)));
        out.end_seq();

	out << YAML::Key << "edges" << YAML::Value;
        out.begin_seq();
		    adobe::for_each( edges(), boost::bind( &composition_t::write_edge, this, boost::ref( out), _1));
		out.end_seq();
}
