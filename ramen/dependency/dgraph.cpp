...
#include<ramen/dependency/dgraph.hpp>
...
dgraph_t::dgraph_t() : valid_order_( true) {}
dgraph_t::~dgraph_t() {}

void dgraph_t::add_node( dnode_t *v)
{
...
void dgraph_t::remove_node( dnode_t *v)
{
	RAMEN_ASSERT( has_node( v));
...
bool dgraph_t::has_node( dnode_t *v) const
{
	return node_to_desc_.find( v) != node_to_desc_.end();
}

void dgraph_t::add_dependency( dnode_t *s, dnode_t *d)
{
...
void dgraph_t::remove_dependency( dnode_t *s, dnode_t *d)
{
	RAMEN_ASSERT( s != d);
...
void dgraph_t::set_node_and_dependencies_dirty( dnode_t *n)
{
	n->set_dirty( true);
	...
	for( boost::tie( it, ie) = boost::out_edges(v, graph_); it != ie; ++it)
	{
		...
		dnode_t *nn = graph_[dst].node;
		set_node_and_dependencies_dirty( nn);
	}
}

void dgraph_t::notify_all_dirty()
{
	if( !valid_order_)
	{
