...
{
	if( b)
		flags_ |= can_have_expressions_bit;
	else
		flags_ &= ~can_have_expressions_bit;
}
...
void param_t::make_paths_absolute() {}
void param_t::make_paths_relative() {}

// util
void param_t::apply_function( const boost::function<void ( param_t*)>& f) { f( this);}
...
// serialization
void param_t::read( const serialization::yaml_node_t& node) { do_read( node);}
...
void param_t::do_read( const serialization::yaml_node_t& node)
{
