...
void aspect_ratio_param_t::do_add_to_hash( hash_generator_t& hash_gen) const { hash_gen << get_value<float>( *this);}
...
int aspect_ratio_param_t::index_for_value( float x) const
{
	...
	QWidget *aspect_ratio_param_t::do_create_widgets()
	{
		...
		menu_->move( ui::inspector_t::Instance().left_margin() + s.width() + 5, 0);
