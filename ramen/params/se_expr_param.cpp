...
void se_expr_param_t::do_add_to_hash( hash_generator_t& hash_gen) const
{
...
void se_expr_param_t::do_enable_widgets( bool e)
{
	...
	QWidget *se_expr_param_t::do_create_widgets()
	{
		...
		int width = ui::inspector_t::Instance().width() - ui::inspector_t::Instance().left_margin() - 10;
		int height = 0;

		std::string str = get_value<std::string>( *this);
		multi_input_ = new ui::text_edit_t( top);
		multi_input_->move( ui::inspector_t::Instance().left_margin(), height);
