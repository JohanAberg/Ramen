...
#include<base/ui/nice_numbers.hpp>
...
{
    int new_val = Imath::clamp( v, min_value_, max_value_);
...
int time_scale_t::frame_from_mouse_pos( int x) const
{
    ...
    return Imath::clamp( round_halfup( f), min_value_, max_value_);
}
...
void time_scale_t::paintEvent ( QPaintEvent *event)
{
    ...
    base::ui::nice_numbers_t numbers( min_value_, max_value_, nticks);

    for( base::ui::nice_numbers_t::iterator it( numbers.begin()); it < numbers.end(); ++it)
    {
        float x = *it;
...
{
    int new_value = Imath::clamp( frame, ( int) start_->value(), ( int) end_->value());
