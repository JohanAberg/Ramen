...
void anim_curves_view_t::paintEvent( QPaintEvent *event)
{
    ...
    BOOST_FOREACH( const anim::track_t *t, anim_editor_t::Instance().active_tracks())
    {
...
{
    int start_frame = document_t::Instance().composition().start_frame();
    int end_frame = document_t::Instance().composition().end_frame();
